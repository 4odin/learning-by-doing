	...
	main_loop: for !rl.WindowShouldClose() {
		...
		for i: uint = 0; i < len(state.previous_keys); i += 1 {
			...
			if array_key_pos(&current_keys, key) == -1 {
				#partial switch key {
				case rl.KeyboardKey.LEFT_SHIFT:
					mu.input_key_up(ctx, .SHIFT)
				case rl.KeyboardKey.RIGHT_SHIFT:
					mu.input_key_up(ctx, .SHIFT)
				case rl.KeyboardKey.LEFT_CONTROL:
					mu.input_key_up(ctx, .CTRL)
				case rl.KeyboardKey.RIGHT_CONTROL:
					mu.input_key_up(ctx, .CTRL)
				case rl.KeyboardKey.LEFT_ALT:
					mu.input_key_up(ctx, .ALT)
				case rl.KeyboardKey.RIGHT_ALT:
					mu.input_key_up(ctx, .ALT)
				case rl.KeyboardKey.ENTER:
					mu.input_key_up(ctx, .RETURN)
				case rl.KeyboardKey.KP_ENTER:
					mu.input_key_up(ctx, .RETURN)
				case rl.KeyboardKey.BACKSPACE:
					mu.input_key_up(ctx, .BACKSPACE)
		...
		for i: uint = 0; i < len(current_keys); i += 1 {
			...
			if array_key_pos(&state.previous_keys, key) == -1 {
				...
				case rl.KeyboardKey.ESCAPE:
					break main_loop
				case rl.KeyboardKey.LEFT_SHIFT:
					mu.input_key_down(ctx, .SHIFT)
				case rl.KeyboardKey.RIGHT_SHIFT:
					mu.input_key_down(ctx, .SHIFT)
				case rl.KeyboardKey.LEFT_CONTROL:
					mu.input_key_down(ctx, .CTRL)
				case rl.KeyboardKey.RIGHT_CONTROL:
					mu.input_key_down(ctx, .CTRL)
				case rl.KeyboardKey.LEFT_ALT:
					mu.input_key_down(ctx, .ALT)
				case rl.KeyboardKey.RIGHT_ALT:
					mu.input_key_down(ctx, .ALT)
				case rl.KeyboardKey.ENTER:
					mu.input_key_down(ctx, .RETURN)
				case rl.KeyboardKey.KP_ENTER:
					mu.input_key_down(ctx, .RETURN)
				case rl.KeyboardKey.BACKSPACE:
					mu.input_key_down(ctx, .BACKSPACE)
				case: // other cases are handled in section "handle_text_input"
				}
			}
		}
...
render :: proc(ctx: ^mu.Context) {
	...
	rl.BeginDrawing()
	{
		...
		rl.DrawTextureRec(
			...
			rl.Rectangle{0, 0, f32(state.screen_width), -f32(state.screen_height)},
			rl.Vector2{0, 0},
			rl.WHITE,
		)
	}
